...
/// Minimal Version-4 (USLP) transfer frame model for Gateway 3.
///
/// Like `Version3Frame`, this is a compact header model suitable for unit testing and later
/// replacement with exact CCSDS 211.0-B-5 offsets.
///
/// Header layout used here (byte-aligned):
/// - byte 0:
///   - bits 7..6: version = 0b11
///   - bit 5: kind (0=P, 1=U)
...
impl Version4Frame {
    pub(crate) fn from_bytes_without_crc(data: &[u8]) -> Result<Self, FrameError> {
        if data.len() < 4 {
            return Err(FrameError::Truncated("v4 header too short"));
        }
        let b0 = data[0];
        let version = b0 >> 6;
        if version != 0b11 {
